...
## 📦 Установка пакетов после переустановки

Полные списки приложений лежат в [`packages/`](packages/):

- `repo.txt` / `aur.txt` — всё явно установленное (official + AUR)
- `rice-repo.txt` / `rice-aur.txt` — урезанный rice-набор
- `./packages/install.sh` — поставить всё; `./packages/install.sh --rice` — только rice
- `./packages/export.sh` — обновить списки после новых установок

Конфиги (Hyprland, Kanata, Kitty, Fish, Tmux, Zen shortcuts, theme SSOT, …) уже в этом репозитории → `./restow.sh`.
