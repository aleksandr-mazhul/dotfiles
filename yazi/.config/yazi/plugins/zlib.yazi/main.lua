--- @since 25.5.31
-- Preview zlib / Git loose objects without invoking git.
...
local function preview_script()
	...
	return conf .. "/plugins/zlib.yazi/preview.py"
end
...
function M:peek(job)
	...
	repeat
		...
		i = i + 1
		if i > job.skip then
			-- strip trailing newline; keep the rest intact (spaces matter)
	...
	-- Wrap long commit/blob lines so the panel edge doesn't clip mid-word.
